...
class DefaultServerSpecification extends Specification {
    ...
    def 'invalidate should invoke server listeners'() {
        given:
        def serverMonitor = new TestServerMonitor()
        def serverListener = Mock(ServerListener)
        def server = defaultServer(Mock(ConnectionPool), serverMonitor, serverListener)
    ...
    def 'should enable command listener'() {
        ...
        def server = defaultServer(Mock(ConnectionPool), Mock(ServerMonitor), Mock(ServerListener), commandListener)
        def testConnection = (TestConnection) server.getConnection()
    ...
    private DefaultServer defaultServer(final ConnectionPool connectionPool, final ServerMonitor serverMonitor) {
        defaultServer(connectionPool, serverMonitor, Mock(ServerListener))
    }

    private DefaultServer defaultServer(final ConnectionPool connectionPool, final ServerMonitor serverMonitor,
                                        final ServerListener serverListener) {
        defaultServer(connectionPool, serverMonitor, serverListener, Mock(CommandListener))
    }

    private DefaultServer defaultServer(final ConnectionPool connectionPool, final ServerMonitor serverMonitor,
                                        final ServerListener serverListener, final CommandListener commandListener) {
        def sdam = new DefaultSdamServerDescriptionManager(serverId, Mock(ServerDescriptionChangedListener), serverListener,
                serverMonitor, connectionPool)
